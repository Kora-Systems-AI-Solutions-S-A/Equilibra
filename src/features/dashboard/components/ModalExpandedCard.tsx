...
export const ModalExpandedCard = () => {
  ...
  const renderIncomeContent = () => {
    ...
    const projection = getProjectedNextMonthIncome(filteredRecords);
  ...
  const renderMonthlySummaryContent = () => {
    const finalFilteredRecords = filteredRecords.filter(t => {
      if (summaryFilter === 'Todos') return true; // filterRecordsByDashboardFilters already removes Cancelado
